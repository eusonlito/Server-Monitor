...
abstract class ControllerWebAbstract extends ControllerAbstract
{
     ...
     * @return mixed
     */
    final protected function actionIfExists(string|array $names, ...$args): mixed
    {
        $action = $this->request->input('_action');

        foreach ((array)$names as $name) {
            if ($name === $action) {
                return $this->actionCall($name, ...$args);
            }
        }
     ...
     * @return mixed
     */
    final protected function actionPost(string|array $names, ...$args): mixed
    {
        if ($this->request->isMethod('post') === false) {
            return null;
        }

        return $this->actionIfExists($names, ...$args);
    }
