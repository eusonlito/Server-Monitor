...
<div class="box flex items-center px-5">
    <div class="nav nav-tabs flex overflow-auto whitespace-nowrap" role="tablist">
        ...
        <a href="{{ route('server.update.setup', $row->id) }}" class="p-4 {{ ($ROUTE === 'server.update.setup') ? 'active' : '' }}" role="tab">{{ __('server-update.header.setup') }}</a>
        ...
        <a href="{{ route('server.update.measure-disk', $row->id) }}" class="p-4 {{ ($ROUTE === 'server.update.measure-disk') ? 'active' : '' }}" role="tab">{{ __('server-update.header.disks') }}</a>
